document.addEventListener('DOMContentLoaded', () => {
  ...
  const toggle = document.querySelector('.tamin-exporter-toggle');
  const colCodes = document.querySelectorAll('.tamin-exporter-col-code');
  ...
  const setAdvanced = (enabled) => {
    colCodes.forEach((code) => {
      code.style.display = enabled ? 'inline' : 'none';
    });
  };
